...
import {
  ...
  Trash2, RefreshCw, Shield, CalendarDays, Hourglass,
} from 'lucide-react';
...
function Btn({ children, onClick, bg = '#000', color = '#FFE500', disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
...
export default function SuperAdminDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    try { setStats(await getSuperAdminStats()); } catch { toast.error('Failed to load stats'); }
  }, []);

  const fetchRequests = useCallback(async () => {
    try { setRequests(await getSuperAdminRequests()); } catch { toast.error('Failed to load requests'); }
  }, []);

  const fetchAdmins = useCallback(async () => {
    try { setAdmins(await getSuperAdminAdmins()); } catch { toast.error('Failed to load admins'); }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchRequests();
    fetchAdmins();
  }, [fetchStats, fetchRequests, fetchAdmins]);
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'stats' && (
            <motion.div key="stats" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <h1 style={{ fontWeight: 900, fontSize: '1.8rem', textTransform: 'uppercase', marginBottom: '0.3rem' }}>
                Platform Stats
              </h1>
          ...
          {activeTab === 'requests' && (
            <motion.div key="requests" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '0.3rem' }}>
                <h1 style={{ fontWeight: 900, fontSize: '1.8rem', textTransform: 'uppercase', margin: 0 }}>
                  Pending Requests
                </h1>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: '#000', color: '#FFE500' }}>
                        {['Username', 'Email', 'Events Created', 'Joined', 'Actions'].map((h) => (
                          <th key={h} style={{ padding: '0.9rem 1rem', textAlign: 'left', fontSize: '0.72rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '1.5px', whiteSpace: 'nowrap' }}>
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {admins.map((admin, i) => (
                        <tr
                          key={admin._id}
                          style={{ borderBottom: '2px solid #000', background: i % 2 === 0 ? '#fff' : '#FAFAF5' }}
                        >
                          <td style={{ padding: '0.85rem 1rem', fontWeight: 900, textTransform: 'uppercase', fontSize: '0.85rem' }}>
                            {admin.username}
                          </td>
                          <td style={{ padding: '0.85rem 1rem', fontSize: '0.82rem', fontWeight: 600, opacity: 0.7 }}>
                            {admin.email}
                          </td>
                          <td style={{ padding: '0.85rem 1rem' }}>
                            <span style={{ background: '#FFE500', border: '2px solid #000', padding: '2px 10px', fontWeight: 900, fontSize: '0.85rem' }}>
                              {admin.totalEventsCreated}
                            </span>
                          </td>
                          <td style={{ padding: '0.85rem 1rem', fontSize: '0.75rem', fontWeight: 700, opacity: 0.5 }}>
                          ...
                          <td style={{ padding: '0.85rem 1rem' }}>
                            <Btn
                              ...
                              disabled={loadingAction === 'del' + admin._id}
                            >
